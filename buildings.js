...
map.on('click', (e) => {
  ...
  const building = {
    ...
    personnel: (type === 'cpi' || type === 'cs' || type === 'csp')
      ? { pro: 0, vol: 0 }
      : 0,
  ...
  const labelPrefix = type === 'hopital' ? 'CH' : type === 'police' ? 'Commissariat' : 'CIS';
        ...
        <strong>
          ...
          ${
            (type === 'cpi' || type === 'cs' || type === 'csp')
              ...
              `
              : `(<span id="staff-avail-${safeId}">0</span>/<span id="staff-${safeId}">0</span>)`
          }
          ${type === 'hopital' ? `<p>🛏 Patients : <span id="capacity-${safeId}">0/0</span></p>` : ''}
        </strong>
  ...
  `)
  .bindTooltip(`${labelPrefix} ${name}`, { permanent: false, direction: 'top' });
...
function addVehicle(safeId, vehicleType) {
  ...
  if (building.vehicles.length >= limit) {
    alert(`Limite atteinte : ce bâtiment ne peut accueillir que ${limit} véhicules.`);
    return;
  }
  ...
  const count = building.vehicles.filter(v => v.type === vehicleType).length + 1;
...
function openHospitalModal(buildingId) {
  ...
  const reserved = hospital.reservedPatients ? hospital.reservedPatients.length : 0;
  const totalPatients = (hospital.patients?.length || 0) + reserved;

  document.getElementById('hospital-name').textContent = `${hospital.name} (${totalPatients}/${hospital.capacity} patients)`;
  ...
  hospital.patients.forEach(p => {
    const timeLeft = Math.max(0, Math.ceil((p.duration - (now - p.entryTime)) / 1000));
    const li = document.createElement('li');
    const patientName = p.name || `Victime (${PATIENT_STATUS_FR[p.severity] || p.severity})`;
  ...
  if (reserved > 0) {
    hospital.reservedPatients.forEach(r => {
      const reservedName = r.name || `Victime (${PATIENT_STATUS_FR[r.severity] || r.severity})`;
...
function recruit(safeId, recruitType = 'pro') {
  ...
  if (building.type === 'cpi' || building.type === 'cs' || building.type === 'csp') {
    ...
    if (recruitType === 'pro') {
      ...
      building.personnelAvailablePro = (building.personnelAvailablePro || 0) + 1;
    } else {
      building.personnelVol = (building.personnelVol || 0) + 1;
      building.personnelAvailableVol = (building.personnelAvailableVol || 0) + 1;
    }
  } else {
    // Autres bâtiments
  ...
  if (modalStaff) {
    if (building.type === 'cpi' || building.type === 'cs' || building.type === 'csp') {
      modalStaff.innerHTML = `<strong>Personnel actuel :</strong>
        Pro ${building.personnelPro || 0} (dispo ${building.personnelAvailablePro || 0}) /
        Vol ${building.personnelVol || 0} (dispo ${building.personnelAvailableVol || 0})`;
    } else {
      modalStaff.innerHTML = `<strong>Personnel actuel :</strong> ${building.personnelAvailable || 0}/${building.personnel || 0}`;
    }
  }
...
function refreshBuildingStaffDisplay(building) {
  const safeId = getSafeId(building);
  if (building.type === 'cpi' || building.type === 'cs' || building.type === 'csp') {
    document.getElementById(`staff-pro-${safeId}`).textContent = building.personnelPro;
    document.getElementById(`staff-vol-${safeId}`).textContent = building.personnelVol;
    document.getElementById(`staff-pro-avail-${safeId}`).textContent = building.personnelAvailablePro || 0;
    document.getElementById(`staff-vol-avail-${safeId}`).textContent = building.personnelAvailableVol || 0;
  } else {
    document.getElementById(`staff-${safeId}`).textContent = building.personnel;
    document.getElementById(`staff-avail-${safeId}`).textContent = building.personnelAvailable || 0;
  }
}
...
function updateSidebarVehicleList(safeId) {
  ...
  building.vehicles.forEach(v => {
    ...
    li.innerHTML = `
      <span>${v.label}</span>
      <span class="status ${v.status}" title="${v.status.toUpperCase()}">${v.status.toUpperCase()}</span>
    `;
    list.appendChild(li);
...
function refreshVehicleStatusForBuilding(building) {
  ...
  building.vehicles.forEach(vehicle => {
    if (['er', 'al', 'tr', 'ch', 'ot', 'at', 'hs'].includes(vehicle.status)) return;
    if (building.type === 'cpi' || building.type === 'cs' || building.type === 'csp') {
...
document.getElementById('building-search').addEventListener('input', applyFilters);
